...
bool Ngx_Mem_Pool::ngx_create_pool()
{
    ...
    //以页大小作为内存块的大小
    m_pool->small = (ngx_pool_data_t*)ngx_memalign(pagesize, NGX_POOL_ALIGNMENT);
    if (m_pool->small == nullptr) {
        return false;
    }

    m_pool->small->last = (u_char*)m_pool->small + sizeof(ngx_pool_data_t);
    m_pool->small->end = (u_char*)m_pool->small + pagesize;
    m_pool->small->next = nullptr;
    m_pool->small->failed = 0;

    m_pool->max = pagesize - sizeof(ngx_pool_data_t);    
...
void Ngx_Mem_Pool::ngx_reset_pool()
{
    ...
    //重置小内存块
    for (s = m_pool->small; s; s = s->next) {
        s->last = (u_char*)s + sizeof(ngx_pool_data_t);
...
void* Ngx_Mem_Pool::ngx_palloc(size_t size) //申请内存（对齐）
{   
    spin_lock.lock();
    void* p = nullptr;
    if (size <= m_pool->max) {
...
void* Ngx_Mem_Pool::ngx_pnalloc(size_t size) //申请内存（不对齐）
{
    spin_lock.lock();
    void* p = nullptr;
    if (size <= m_pool->max) {
...
void* Ngx_Mem_Pool::ngx_pcalloc(size_t size) //申请内存（对齐并初始化）
{
    void *p;
    p = ngx_palloc(size);
    if (p) {
